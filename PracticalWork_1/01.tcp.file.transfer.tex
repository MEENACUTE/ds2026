...
This report presents a complete implementation of file transfer over TCP/IP using command-line interface (CLI) and Python's built-in \texttt{socket} library. The system allows a client to request and download any file from the server reliably.

\section{Protocol Design}
The protocol is simple, reliable, and follows a request-response pattern over TCP:

\begin{itemize}
    \item Client connects to server at \texttt{127.0.0.1:65432}
    \item Client sends the requested filename (UTF-8 string)
    \item Server checks if file exists:
    \begin{itemize}
        \item If not → sends \texttt{"File not found"}
        \item If yes → sends file size (as string), then file content in 4096-byte chunks
    \end{itemize}
    \item Client receives and writes data to local file
    \item Connection closes automatically after transfer (TCP EOF)
\end{itemize}

\begin{figure}[H]
    \centering
    \includesvg[width=0.95\textwidth]{PracticalWork_1/protocol.svg}
    \caption{Sequence Diagram - TCP File Transfer Protocol}
    \label{fig:protocol}
\end{figure}

\section{System Organization}
The system follows classic client-server architecture:
...
\begin{figure}[H]
    \centering
    \includesvg[width=0.8\textwidth]{PracticalWork_1/system.svg}
    \caption{Component Diagram - Client-Server System Organization}
    \label{fig:system}
\end{figure}

\section{File Transfer Implementation}
Implementation uses only standard Python libraries. Run server first, then client in a separate terminal.
...
\begin{itemize}
    \item \textbf{Server}:
    \begin{itemize}
        \item Creates and binds socket to port 65432
    ...
    \begin{itemize}
        ...
        \item Sends desired filename
        \item Receives file size and data
        \item Writes to local file with progress tracking
